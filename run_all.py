...
Usage:
    python run_all.py                  # Run all analyses for all groups
...
def run_script(script: str, group: str) -> bool:
    """Run a single analysis script for a given group. Returns True on success."""
    print(f"\n{'='*60}")
    print(f"Running: {script} --group {group}")
    print(f"{'='*60}")
    result = subprocess.run(
        [sys.executable, script, "--group", group],
...
def main():
    ...
    for group in groups:
        ...
        for script in scripts:
            key = f"{script} --group {group}"
            results[key] = run_script(script, group)
