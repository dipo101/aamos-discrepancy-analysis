...
def test_world_ids_round_trip():
    ...
    assert BASELINE.world_id == "span=Q__case=A"


@pytest.mark.parametrize("bad", ["Q", "span=Q", "span=Q__case=D", "span=X__case=A", "span=Q__case=B", "span=Q__case=A__k=01", "span=Q__case=B__k=1"])
...
def test_paths_are_keyed_by_world_id(sandbox):
    ...
    assert comparisons_dir(BASELINE, "concordant", "bland_altman") == sandbox / "v2" / "comparisons" / "span=Q__case=A" / "concordant" / "bland_altman"
...
def test_register_world_is_idempotent_and_sorted(sandbox):
    ...
    assert list_worlds() == ["span=D__case=A", "span=Q__case=A"]
...
def test_concordant_sets_merge_and_sort(sandbox):
    ...
    assert list(sets) == ["span=D__case=A", "span=Q__case=A"]
    assert sets["span=D__case=A"]["mean"] == [294, 702]
    assert sets["span=Q__case=A"]["median"] == [294, 473, 702, 917]
